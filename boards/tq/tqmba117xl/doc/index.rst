...
.. Autor: Isaac L. L. Yuki

.. SPDX-License-Identifier: CC-BY-4.0 OR Apache-2.0

.. zephyr:board:: tqmba117xl

Overview
********

The TQMa117xL module features the NXP i.MX RT1170 processor with a Cortex-M7 core
running at up to 1GHz. It provides numerous interfaces including dual Gigabit
Ethernet, making it ideal for applications that require both energy efficiency
...
.. list-table::
   ...
   * - DISPLAY
     ...
     - eLCDIF / MIPI-DSI, tested with
       ``rk055hdmipi4ma0``, ``tqmba117xl_tm070`` shields
   * - FLEXSPI
     - on-chip
     - Flash programming
   * - SDHC
...
``boards/tq/tqmba117xl/tqmba117xl_cm7_defconfig``.
...
:ref:`jlink-debug-host-tools` and ensure they are in your environmental *PATH*.

Either update the on-board debug circuit with Segger J-Link firmware or attach
an external :ref:`jlink-external-debug-probe`.

Ensure the FLEXSPI1 pin-mux is correct configured before debugging or flashing.

Typical device string for J-Link GDB for the flash configuration:
...
7. Select **fuse 0x9A0** and select the wildcard on bit 10.
8. When clicking on the wildcard it changes from "*" to "1". The "Required value" should now be 0x400. Double-check these values.
9. Verify, switch to *Advanced mode*, press *Burn*, and confirm.

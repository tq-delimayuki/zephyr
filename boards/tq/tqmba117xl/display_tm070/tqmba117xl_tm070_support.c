...
#else
#error "I2C Peripheral 5 isn't set."
#endif
...
uint32_t BOARD_PrepareDisplayController(void)
{
	...
	/*init seq 1: enable pwm signal (used display API does not support this feature)*/
	...
	status = pwm_set(pwm_dev, PWM_CHANNEL, PWM_PERIOD, PWM_PERIOD, PWM_POLARITY_NORMAL);
	...
	/* init seq 2: enable LP11 on inactive DSI lanes -> See Reference Manual */
	...
	/* wait for it (Datasheet Spec defined time) */
	k_sleep(K_MSEC(10));

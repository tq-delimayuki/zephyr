...
#define PMIC_LEAST_SIGNIFICANT_BIT_VALUE 0.00625f
#define PMIC_VOLTAGE_OFFSET 0.4f
 ...
 * \param[in] handle	Handle to the PF5020 device.
...
uint32_t PF5020_readCoreVoltage(float *voltage, PF5020_Handle_t *handle)
{
	...
	status = handle->transfer(handle->peripheral, Banks[SW1_RUN_VOLT].address,
				  ...
				  PF5020_READ);
	;
	*voltage = (float)(PMIC_VOLTAGE_OFFSET +
			   ((float)registerValue * PMIC_LEAST_SIGNIFICANT_BIT_VALUE));
	return status;
}
